...
if [ "${CMAKE-}" == true ]; then
    ...
    mkdir -p "${CUR}/build/cmake"
    cd "${CUR}/build/cmake"
    cmake "${CUR}/CMake" ${CMAKE_MAKE} -DBUILD_CursesDialog:BOOL=ON -DCMAKE_BUILD_TYPE:STRING=Release -DCMAKE_INSTALL_PREFIX:PATH="${CUR}/install/cmake"
    ${MAKE} install
    cd "${CUR}"

    export PATH="${CUR}/install/cmake/bin:${PATH}"
    echo $PATH
fi

# fetch sources
if [ "${LLVM-}" == true ]; then
    mkdir -p "${CUR}/build/llvm"
    mkdir -p "${CUR}/build/rv"
    ...
    cd "${CUR}/llvm-project"
    if ! patch --dry-run --reverse --force -s -p1 -i "${CUR}/patches/llvm/nvptx_feature.patch"; then
        patch -p1 -l -i "${CUR}/patches/llvm/nvptx_feature.patch"
    fi
    ...
    cmake "${CUR}/llvm-project/llvm" ${CMAKE_MAKE} -DLLVM_BUILD_LLVM_DYLIB:BOOL=ON -DLLVM_LINK_LLVM_DYLIB:BOOL=ON -DCMAKE_BUILD_TYPE:STRING=${BUILD_TYPE} -DCMAKE_INSTALL_PREFIX:PATH="${CUR}/install/llvm" \
        -DLLVM_ENABLE_PROJECTS:STRING="clang;lld" -DLLVM_ENABLE_BINDINGS:BOOL=OFF -DLLVM_INCLUDE_TESTS:BOOL=ON -DLLVM_TARGETS_TO_BUILD:STRING="${LLVM_TARGETS}" -DDEFAULT_SYSROOT:PATH="${DEFAULT_SYSROOT}"
    ${MAKE} install
    cd "${CUR}"

    LLVM_VARS=-DLLVM_DIR:PATH="${CUR}/install/llvm/lib/cmake/llvm"

    if [ "${RV-}" == true ]; then
        cd "${CUR}/build/rv"
        cmake "${CUR}/llvm-project/rv" ${CMAKE_MAKE} ${LLVM_VARS} -DBUILD_SHARED_LIBS:BOOL=ON -DCMAKE_BUILD_TYPE:STRING=${BUILD_TYPE} -DRV_REBUILD_GENBC:BOOL=${RV_REBUILD_GENBC} -DCMAKE_INSTALL_PREFIX="${CUR}/install/llvm" -DDEFAULT_SYSROOT:PATH="${DEFAULT_SYSROOT}"
        ${MAKE}
        RV_VARS=-DRV_DIR:PATH="${CUR}/build/rv/share/anydsl/cmake"
        cd "${CUR}"
    else
...
export PATH="${CUR}/install/llvm/bin:${CUR}/artic/build/bin:${CUR}/impala/build/bin:\${PATH:-}"
export LD_LIBRARY_PATH="${CUR}/install/llvm/lib:\${LD_LIBRARY_PATH:-}"
_EOF_
if [ "${CMAKE-}" == true ]; then
    echo "export PATH=\"${CUR}/install/cmake/bin:\${PATH:-}\"" >> ${CUR}/project.sh
fi
...
if [ "${BUILD_PAL-}" == true ]; then
    ...
    mkdir -p "${CUR}/build/pal"
    cd "${CUR}/build/pal"
    cmake "${CUR}/pal" ${CMAKE_MAKE} -DCMAKE_BUILD_TYPE:STRING=${BUILD_TYPE} -DCMAKE_POSITION_INDEPENDENT_CODE="ON" \
        -DPAL_CLIENT_INTERFACE_MAJOR_VERSION=925 -DPAL_CLIENT:STRING="Vulkan" -DPAL_BUILD_GFX:STRING="ON" \
        -DPAL_BUILD_NULL_DEVICE:STRING="ON" -DPAL_BUILD_OSS:STRING="ON" -DPAL_BUILD_GFX9:STRING="ON" -DPAL_BUILD_GFX11:STRING="ON" -DPAL_BUILD_GFX12:STRING="ON" \
        -DPAL_BUILD_HAWK_POINT1:STRING="ON" -DPAL_BUILD_HAWK_POINT2:STRING="ON" -DPAL_BUILD_NAVI48:STRING="ON" -DPAL_BUILD_STRIX_HALO:STRING="ON" \
        -DPAL_DEVELOPER_BUILD:STRING="ON" -DPAL_AMDGPU_BUILD:STRING="ON" -DPAL_BUILD_RDF:STRING="ON"
    ${MAKE}
    ...
    mkdir -p "${CUR}/build/rocm-device-libs"
    cd "${CUR}/build/rocm-device-libs"
    cmake "${CUR}/llvm-project/amd/device-libs" ${CMAKE_MAKE} -DCMAKE_PREFIX_PATH:PATH="${CUR}/build/llvm" -DAMDGPU_TARGET_TRIPLE:STRING="amdgcn-amd-amdpal" -DCLANG_OPTIONS_APPEND:STRING="-mcpu=${ROCm_GPU_GENERATION}"
    ${MAKE}
    # change calling convention to amdgpu_gfx for calls to ocml
    cd "${CUR}/build/rocm-device-libs/amdgcn/bitcode"
    "${CUR}/install/llvm/bin/llvm-dis" ocml.bc -o ocml.ll
    python3 "${CUR}/scripts/rocm-device-libs-cc-patcher.py"
    PAL_VARS=-Dpal_DIR:PATH="${CUR}/build/pal"
else
    PAL_VARS=
fi
